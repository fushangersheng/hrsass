      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <ImageUpload ref="avatar"></ImageUpload>
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <ImageUpload ref="person"></ImageUpload>
        </el-form-item>
  ...
  methods: {
    async saveUser() {
      ...
      if (fileList.some((item) => !item.upload)) {
        this.$message.warning("图片没有上传成功，请重新上传");
      }
    ...
    async savePersonal() {
      const fileList = this.$refs.person.fileList;
      if (fileList.some((item) => !item.upload)) {
        this.$message.warning("图片没有上传成功，请重新上传");
      }
